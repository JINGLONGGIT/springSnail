 ...
 * Description: HTTP请求管理类定义
 * Author: jinglong
 * Date: 2020年5月4日 16:28
 * History: 
 *****************************************************************/
...
/*************************************************************************
 * 函数名称：addfd
 * 函数功能：注册文件描述符fd的内核事件表
 * 输入参数：int epollfd:          内核事件表的文件描述符，由epoll_create()函数生成
 *           int fd:            要注册的文件描述符
 *           bool one_shot      是否为注册EPOLLONESHOT事件
 * 输出参数：无
 * 返 回 值：无
 *************************************************************************/
 ...
 * 函数名称：init
 * 函数功能：初始化新接收到的HTTP连接
 * 输入参数：int sockfd                     客户连接socket
 *           const sockaddr_in & addr       客户端地址
 * 输出参数：无
 * 返 回 值：无
 *************************************************************************/
...
// 当遇到一个完整的、正确的HTTP请求时，我们就分析目标文件的属性。如果目标文件存在，对所有用户可读，
...
bool http_conn::write()
{
    ...
    while (1)
    {
        ...
        if (temp <= -1)
        {
            // 如果TCP写缓存没有空间，则等待一下轮的EPOLLOUT事件
            if (errno == EAGAIN)
            {
